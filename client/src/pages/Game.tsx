...
const T = {
  ...
  subtitle: "Binoni qurating. Qavatlari mukammal joylashtirib, 50% gacha chegirma kazaning.",
  menuTitle: "O'yinga tayyor?",
  menuSubtitle: "6 qavat qurating va mukofot yuting.",
  namePlaceholder: "Mehmonning nomini kiriting",
  gestureLabel: "Qo'l harakati bilan boshqarish",
  startButton: "O'YINNI BOSHLASH",
  viewLeaderboard: "Yo'g'orlash ro'yxatini ko'ring",
  topBuilders: "Eng yaxshi quriluvchilar",
  how: "O'yin qoidalari",
  ...
  gameOverSubtitle: "Minora beqaror bo'ldi.",
  floors: "Qavatlari",
  ...
  winSubtitle: "Siz mehmonxonani muvaffaqiyatli quradingiz.",
  discountEarned: "Yutgan chegirma",
  discountBased: "Qavat joylashtirishning aniqligi asosida",
        ...
        <div className="hidden lg:flex flex-col gap-6 h-full justify-center">
          ...
          <Card className="p-6 bg-white/80 backdrop-blur border-none shadow-xl flex-1 max-h-[400px] overflow-hidden flex flex-col">
            ...
            <div className="overflow-y-auto space-y-3 flex-1 pr-2">
              ...
              {!leaderboard?.length && (
                <div className="text-center text-muted-foreground py-8">Hali rekord yo'q. Siz birinchi bo'ling!</div>
              )}
            </div>
          </Card>
        </div>
          ...
          <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-2xl border-[8px] border-white bg-gradient-to-b from-[#e6f3ff] to-white">
            ...
            <AnimatePresence>
              ...
              {gameState === 'MENU' && (
                ...
                >
                  <div className="space-y-2">
                    <h2 className="text-4xl font-display font-bold text-primary">{T.menuTitle}</h2>
                  ...
                  <Button 
                    ...
                    onClick={startGame}
                  >
                    <Play className="w-5 h-5 mr-2 fill-current" /> {T.startButton}
                  </Button>
                  
                  {/* Mobile Leaderboard Trigger */}
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="ghost" className="lg:hidden w-full">{T.viewLeaderboard}</Button>
                    </DialogTrigger>
                    <DialogContent>
                      <DialogHeader><DialogTitle>{T.topBuilders}</DialogTitle></DialogHeader>
              ...
              {gameState === 'GAME_OVER' && (
                ...
                >
                  <h2 className="text-4xl font-display font-bold mb-2 text-red-500">{T.gameOverTitle}</h2>
                  <p className="text-gray-300 mb-8">{T.gameOverSubtitle}</p>
                  
                  <div className="grid grid-cols-2 gap-4 w-full mb-8">
                    <div className="bg-white/10 p-4 rounded-xl">
                      <div className="text-xs text-gray-400 uppercase tracking-wider">{T.floors}</div>
                      <div className="text-3xl font-bold">{gameResult?.parts}/6</div>
                    </div>
                    <div className="bg-white/10 p-4 rounded-xl">
                      <div className="text-xs text-gray-400 uppercase tracking-wider">{T.stability}</div>
                  ...
                  {gameResult && (
                    <div className="bg-white/10 p-4 rounded-xl w-full mb-8 border border-white/10">
                      <div className="text-sm text-blue-200 uppercase tracking-wider mb-1">{T.discountEarned}</div>
                      <div className="text-4xl font-bold text-[#9D8848]">{gameResult.discount}%</div>
                    </div>
                  )}
              ...
              {gameState === 'WIN' && (
                ...
                >
                  ...
                  <h2 className="text-5xl font-display font-bold mb-2 text-white">{T.winTitle}</h2>
                  ...
                  <div className="bg-white/10 p-6 rounded-2xl w-full mb-8 border border-white/10">
                    ...
                    <div className="text-xs text-white/50 mt-2">{T.discountBased}</div>
                  </div>
          ...
          {useGestures && (
            <Card className="p-4 bg-black border-none shadow-xl overflow-hidden flex flex-col items-center">
              <h3 className="text-white text-sm font-medium mb-3 w-full flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"/> Kamera uzatishi
              </h3>
              <GestureController enabled={useGestures && gameState === 'PLAYING'} onDrop={handleDrop} />
              <p className="text-xs text-gray-500 mt-2">Blokni tushirish uchun <span className="text-white font-bold">qo'lgani yumting</span></p>
            </Card>
          )}
